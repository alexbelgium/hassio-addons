...
case "$(bashio::config "graphic_driver")" in
  x64_AMD)
    if [[ "$BUILD_ARCH" !== amd64 ]]; then bashio::log.fatal "Wrong architecture, $graphic_driver doesn't support $addon_arch"
  ...
  aarch64_rpi)
    if [[ "$BUILD_ARCH" !== arm64 ]]; then bashio::log.fatal "Wrong architecture, $graphic_driver doesn't support $addon_arch"
