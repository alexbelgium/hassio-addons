
## 1.2.4 (2026-04-23)
- Migrate deprecated Home Assistant map names to their app equivalents.
- Update to latest version from benbusby/whoogle-search (changelog : https://github.com/benbusby/whoogle-search/releases)

## 1.2.3 (2026-03-28)
- Update to latest version from benbusby/whoogle-search (changelog : https://github.com/benbusby/whoogle-search/releases)

## 1.2.2 (2026-01-03)
- Update to latest version from benbusby/whoogle-search (changelog : https://github.com/benbusby/whoogle-search/releases)

## 1.1.2 (2025-11-29)
- Update to latest version from benbusby/whoogle-search (changelog : https://github.com/benbusby/whoogle-search/releases)
- Added support for configuring extra environment variables via the `env_vars` add-on option alongside config.yaml. See https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon-2 for details.

## 1.1.0 (2025-10-11)
- Update to latest version from benbusby/whoogle-search (changelog : https://github.com/benbusby/whoogle-search/releases)
## 0.9.4-2 (2025-09-30)
- Applied new homeassistant config logic, which will allow in the future all Whoogle data to be backed up with the add-on. All data (+ config) is moved to /app_configs/db21ed7f_whoogle_search

## 0.9.4 (2025-09-20)
- Update to latest version from benbusby/whoogle-search (changelog : https://github.com/benbusby/whoogle-search/releases)

## 0.9.3 (2025-01-25)
- Update to latest version from benbusby/whoogle-search (changelog : https://github.com/benbusby/whoogle-search/releases)

## 0.9.1 (2024-11-02)
- Update to latest version from benbusby/whoogle-search (changelog : https://github.com/benbusby/whoogle-search/releases)

## 0.9.0 (2024-10-05)
- Update to latest version from benbusby/whoogle-search (changelog : https://github.com/benbusby/whoogle-search/releases)

## 0.8.4 (2023-11-04)

- Update to latest version from benbusby/whoogle-search
## 0.8.3-2 (2023-08-26)

- Minor bugs fixed

## 0.8.3 (2023-08-26)

- Update to latest version from benbusby/whoogle-search
## 0.8.2-5 (2023-05-04)

- Minor bugs fixed
## 0.8.2-4 (2023-05-04)

- Minor bugs fixed
- Fix missing images in direct access

## 0.8.2-3 (2023-05-03)

- Minor bugs fixed
- Fix missing images in ingress

## 0.8.2-2 (2023-04-09)

- Minor bugs fixed

## 0.8.2 (2023-04-08)

- Update to latest version from benbusby/whoogle-search

## 0.8.1 (2022-12-25)

- Update to latest version from benbusby/whoogle-search
- WARNING : update to supervisor 2022.11 before installing

## 0.8.0 (2022-11-26)

- Update to latest version from benbusby/whoogle-search

## 0.7.4 (2022-06-14)

- Update to latest version from benbusby/whoogle-search

## 0.7.3 (2022-06-04)

- Update to latest version from benbusby/whoogle-search
- Addition of ingress

## 0.7.2 (2022-04-27)

- Update to latest version from benbusby/whoogle-search
- Add codenotary sign

## 0.7.1 (2022-01-27)

- Update to latest version from benbusby/whoogle-search

## 0.7.1 (2022-01-03)

- Breaking change
- Changed the config handeling
- Added a docs with the config explaination.
- Make sure you redo your configuration!

## 0.7.0

- Initial release (latest builds)
